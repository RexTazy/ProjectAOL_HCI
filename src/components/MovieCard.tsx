  ...
  return (
    <>
      <Link to={`/movie/${movie.id}`}>
        <Card 
          className="overflow-hidden hover:shadow-xl transition-all duration-300 hover:scale-105 cursor-pointer group relative"
          style={{ 
        ...
        >
          ...
          >
            ...
            {/* Hover overlay with details - Optimized for mobile */}
            <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-80 transition-all duration-300 flex flex-col justify-end p-2 md:p-4">
              <div className="transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                ...
                <div className={`flex ${isUpcoming ? 'justify-center' : 'flex-col'} gap-1 md:gap-2`}>
                  {!isUpcoming && (
                    <Button 
                      className="w-full bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white border-0 text-xs md:text-sm py-1 md:py-2" 
                      size="sm"
                      asChild
                    >
                      <Link to={`/movie/${movie.id}`}>
                  ...
                  <Button 
                    className={`${isUpcoming ? 'w-full' : 'w-full'} bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white border-0 text-xs md:text-sm py-1 md:py-2`}
                    size="sm"
          ...
          <CardContent className="p-2 md:p-3 bg-white absolute bottom-0 left-0 right-0 group-hover:translate-y-full transition-transform duration-300">
            <h3 className="font-semibold text-xs md:text-sm mb-1 line-clamp-1 text-center">{movie.title}</h3>
            <p className="text-gray-600 text-xs text-center">{movie.rating} • {movie.duration}</p>
          </CardContent>
        </Card>
      </Link>
